 ...
 */
export function executeAction(action, dataContext) {
    if (!action)
        return;
    ...
    switch (type) {
        ...
        case "DEEPLINK": {
            const deeplink = resolveVariables(payload.deeplink || payload.url, dataContext);
            if (deeplink)
        ...
        case "NAVIGATE": {
            window.dispatchEvent(new CustomEvent("directo:navigate", {
