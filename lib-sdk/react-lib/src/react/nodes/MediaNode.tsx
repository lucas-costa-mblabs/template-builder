...
export function MediaNode({ node, dataContext }: MediaNodeProps) {
  ...
  const videoIdRef = useRef(
    `directo-video-${Math.random().toString(36).slice(2, 11)}`,
  );
  const [isVisible, setIsVisible] = useState(false);
  const [isPlaying, setIsPlaying] = useState(false);
  const [isMuted, setIsMuted] = useState(node.muted ?? true);

  const width = (node.width as string) || "100%";
  const height = (node.height as string) || "auto";
  ...
  const mediaStyle = useMemo<CSSProperties>(
    () => ({
      ...
      objectFit: mediaType === "video"
        ? "cover"
        : ((node.objectFit as CSSProperties["objectFit"]) || "cover"),
      display: "block",
    ...
    ? (
        ...
        >
          <video
            ...
            ref={videoRef}
            src={!isHlsUrl(url) ? url : undefined}
          ...
          >
            ...
            >
              {isPlaying ? <Pause size={28} /> : <Play size={28} fill="#ffffff" />}
            </span>
          </button>
          <button
            type="button"
            aria-label={isMuted ? "Ativar som" : "Silenciar video"}
            onClick={(event) => {
              event.stopPropagation();
