...
describe("Profile View Modal", () => {
  it("should open profile modal, fetch account feed and toggle follow", async () => {
    ...
    expect(
      await screen.findByRole("dialog", { name: "Perfil de Super Zeus" }),
    ).toBeInTheDocument();
    ...
    expect(
      await screen.findByText("Raquete de Tênis Babolat Pure Aero"),
    ).toBeInTheDocument();
  ...
  it("should handle missing profile account id gracefully", async () => {
    ...
    expect(
      await screen.findByText(
        "Não foi possível identificar o perfil desta conta.",
      ),
    ).toBeInTheDocument();
