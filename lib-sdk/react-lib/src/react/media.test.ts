...
describe("media helpers", () => {
  it("should infer video by explicit type", () => {
    expect(inferMediaType("https://example.com/image.png", "video")).toBe(
      "video",
    );
  });
  ...
  it("should infer video by extension including m3u8", () => {
    ...
    expect(
      inferMediaType("https://example.com/live/playlist.m3u8?token=123"),
    ).toBe("video");
  });

  it("should detect hls URLs", () => {
    expect(isHlsUrl("https://example.com/live/playlist.m3u8#fragment")).toBe(
      true,
    );
